...
if [ -z "$PIP_INDEX_URL" ]; then
	echo "Warning; PIP_INDEX_URL has not been specified."
	echo
fi
...
if [ ! -z "$MissingParam" ]; then
	...
	echo "PIP_INDEX_URL[{9}]: ${9}"
	echo "OUTPUT_IMAGE_TAG[{10}]: ${10}"
	echo "BUILD_CONFIG_POST_FIX[{11}]: ${11}"
    echo "============================================"
	echo
	exit 1
fi
