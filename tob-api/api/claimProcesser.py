...
class ClaimProcesser(object):
    ...
    def __get_VerifiableClaimType(self, claim: ClaimParser):
        ...
        if not verifiableClaimType:
            self.__logger.warn(
                "VerifiableClaimType, {0}, has not been registered ...".format(claim.schemaName))
        else:
            self.__logger.debug(
                "VerifiableClaimType, {0}, exists ...".format(claim.schemaName))
    ...
    def __get_VerifiableOrg(self, claim: ClaimParser):
        ...
        if not verifiableOrg:
            self.__logger.debug(
                "Organization with business id {0} does not exist.".format(organizationId))
        else:
            self.__logger.debug(
                "Organization with business id {0} exists.".format(organizationId))
    ...
    def __get_VerifiableOrgType(self, claim: ClaimParser):
        ...
        verifiableOrgType = VerifiableOrgType.objects.filter(
            orgType=orgTypeCode)
        if not verifiableOrgType:
            self.__logger.debug(
                "VerifiableOrgType, {0}, does not exist.  Creating ...".format(orgTypeCode))
            ...
            self.__logger.debug(
                "VerifiableOrgType, {0}, exists ...".format(orgTypeCode))
    ...
    def __get_Jurisdiction(self, claim: ClaimParser):
        ...
        if not jurisdiction:
            self.__logger.debug(
                "Jurisdiction, {0}, does not exist.  Creating ...".format(jurisdictionName))
            ...
            self.__logger.debug(
                "Jurisdiction, {0}, exists ...".format(jurisdictionName))
    ...
    def __get_LocationType(self, locationTypeName: str):
        ...
        if not locationType:
            self.__logger.debug(
                "LocationType, {0}, does not exist.  Creating ...".format(locationTypeName))
            ...
            self.__logger.debug(
                "LocationType, {0}, exists ...".format(locationTypeName))
    ...
    def __CreateOrUpdateDoingBusinessAs(self, claim: ClaimParser, verifiableOrg: VerifiableOrg):
        ...
        if not doingBusinessAs:
            self.__logger.debug(
                "DoingBusinessAs, {0}, does not exist.  Creating ...".format(dbaName))
    ...
    def __CreateOrUpdateLocation(self, claim: ClaimParser, verifiableOrg: VerifiableOrg, doingBusinessAs: DoingBusinessAs, locationTypeName: str):
        ...
        addlDeliveryInfo = "{0}\r\n{1}".format(
            claim.getField("address_line_2"), claim.getField("country"))
        ...
        location = Location.objects.filter(
            verifiableOrgId=verifiableOrg, postalCode=postalCode)
        if not location:
            self.__logger.debug(
                "Registering new location for {0} ...".format(orgName))
            location = Location(
                verifiableOrgId=verifiableOrg,
            ...
            self.__logger.debug(
                "Updating location for {0} ... ".format(orgName))
            location = location[0]
    ...
    async def __StoreClaim(self, claim):
        ...
        try:
            ...
            self.__logger.debug(
                'Claim for legal_entity_id: %s' % legal_entity_id)
        except Error as e:
            # no-op
    ...
    def SaveClaim(self, claimJson):
        ...
        if "incorporation" in claim.schemaName:
            verifiableOrg = self.__CreateOrUpdateVerifiableOrg(
                claim, verifiableOrg)
            location = self.__CreateOrUpdateLocation(
                claim, verifiableOrg, None, "Headquarters")
        elapsed_time = time.time() - start_time
        ...
        self.__CreateOrUpdateVerifiableClaim(
            claim, verifiableClaimType, verifiableOrg)
        elapsed_time = time.time() - start_time
        self.__logger.debug('Claim elapsed time 3 >>> {}'.format(elapsed_time))
        start_time = time.time()

        eventloop.do(self.__StoreClaim(claim))
        elapsed_time = time.time() - start_time
        ...
        if claim.schemaName == "doing_business_as.bc_registries":
            doingBusinessAs = self.__CreateOrUpdateDoingBusinessAs(
                claim, verifiableOrg)
            location = self.__CreateOrUpdateLocation(
                claim, verifiableOrg, doingBusinessAs, "Location")
        elapsed_time = time.time() - start_time
