  ...
  """
    ...
    def checkUpdateJurisdiction(self, jurisd_def):
        ...
        try:
            ...
            self.__logger.debug(
                "Jurisdiction '{0}' does not exist.  Creating ...".format(
                    jurisd_name
                )
            )
    ...
    def checkUpdateIssuerService(self, jurisdiction, issuer_def):
        ...
        if not issuer:
            self.__logger.debug(
                "Issuer service '{0}' does not exist.  Creating ...".format(
                    issuer_name
                )
            )
    ...
    def checkUpdateClaimTypes(self, issuer, type_defs):
        ...
        for type_def in type_defs:
            ...
            if skey in exist_by_schema:
                claimtype = self.updateRecord(
                    exist_by_schema[skey],
                    {"claimType": type_name, "issuerURL": type_endpoint},
                )
            else:
                self.__logger.debug(
                    "Claim type '{0}' does not exist.  Creating ...".format(
                        type_name
                    )
                )
