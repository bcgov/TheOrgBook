...
export class TopicFormComponent implements OnInit, OnDestroy {
          ...
          if (val && val.data) {
            ...
            if (ids) {
              return this._dataService
                .preloadCredentialTypeLanguage(...Object.keys(ids))
                .then(_ => {
                  resolve(val);
                });
            }
  ...
  get ident(): string {
    ...
    if (source_type && this.source_id) {
      return this.source_type === '_'
        ? this.source_id
        : `ident/${source_type}/${this.source_id}`;
    }
  }
    ...
    for (let s of sections) {
      ...
      if (s === 'all') {
        if (this._loader.result.error) state = true;
          ...
          state = this.isLoaded(
            'topic',
            /*'related_to', */ 'related_to_relations' /*, 'related_from', 'creds'*/
          );
      } else if (s === 'topic') {
